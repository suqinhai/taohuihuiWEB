            ...
            <div v-if="!FormVisible">
                ...
                <el-table :data="tabledata" v-loading="listLoading" @selection-change="selsChange" style="width: 100%;" :max-height="tableHeight" ref="table" @row-click="handleRowClick">
                    ...
                    <el-table-column prop="url" show-overflow-tooltip label="跳转链接">
                        <template scope="scope">
                            <a :href="scope.row.url" target="_blank" style="color: #1f2d3d;"><i class="el-icon-document" style="cursor: pointer;" aria-hidden="true"></i></a>
                        </template>
                    </el-table-column>
        ...
        return {
            ...
            interface: {
                upload: '/taohuihui/public/upload',
                    list: {
                        get: '/taohuihui/poster/get',
                        add: '/taohuihui/poster/add',
                        edit: '/taohuihui/poster/modify',
                        del: '/taohuihui/poster/del',
                        upPoster:'/taohuihui/poster/upPoster',
                        downPoster:'/taohuihui/poster/downPoster'
                    },
            },
            level: 0,
            levelid: [],
    ...
    methods: {
        ...
        handleOnline: function() {
            this.$confirm('确认上线选中记录吗？', '提示', {
                ...
                this.listLoading = true;
                this.$http.post(this.interface.list.upPoster, {
                    _ids: _ids,
                }).then(res => {
                    this.listLoading = false
                    if (res.code == 0) {
        ...
        handleDownline: function() {
            this.$confirm('确认下线选中记录吗？', '提示', {
                ...
                this.listLoading = true;
                this.$http.post(this.interface.list.downPoster, {
                    _ids: _ids,
                }).then(res => {
                    this.listLoading = false
                    if (res.code == 0) {
